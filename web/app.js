...
$("weightSave").onclick=async()=>{const weight=Number($("weightInput").value);if(!weight)return toast("Укажи вес");try{await api("/api/weight",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({telegram_user_id:userId,weight_kg:weight})});toast("Вес записан");loadProgress();}catch(e){toast(e.message)}};
async function loadProgress(){try{const d=await api("/api/progress");if(!d.weights.length)return;const values=d.weights.map(x=>x.weight_kg),min=Math.min(...values)-1,max=Math.max(...values)+1;$("weightHistory").innerHTML=d.weights.map(x=>`<div class="weight-point" title="${x.measured_on}: ${x.weight_kg} кг" style="height:${25+(x.weight_kg-min)/(max-min)*55}px"><span>${x.weight_kg}</span></div>`).join("");}catch{}}
